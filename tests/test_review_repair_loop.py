...
class IntegrationBranchContinuationTests(unittest.TestCase):
    ...
    def test_multi_commit_task_branch_is_cherry_picked_in_order(self) -> None:
        """Cherry-pick merge must apply every task-branch commit, not only the tip."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = _init_repo(root)
            prompt = root / "prompt.md"
            ...
            roadmap_path.write_text(
                json.dumps(
                    {
                        "version": 1,
                        "roadmap_id": "multi-commit-merge",
                        "repo": {"id": "repo", "path": str(repo), "base_branch": "HEAD"},
                        "integration_branch": "integration/agentops-multi-commit",
                        "merge_policy": {"auto_merge": True, "strategy": "cherry_pick"},
                        "tasks": [
                            {
                                "id": "T1",
            ...
            class MultiCommitRunner(BaseRunner):
                name = "multi-commit"
                ...
                def run(self, task, prompt, cwd, artifact_dir, **kwargs):  # type: ignore[no-untyped-def]
                    ...
                    combined_path = artifact / "executor.combined.log"
                    worktree = Path(str(cwd))
                    (worktree / "out.txt").write_text("base\n", encoding="utf-8")
                    git(worktree, "add", "out.txt")
                    git(worktree, "commit", "-m", "base implementation")
                    (worktree / "out.txt").write_text("base\nfollow-up\n", encoding="utf-8")
                    git(worktree, "add", "out.txt")
                    git(worktree, "commit", "-m", "follow-up fix")
                    stdout_path.write_text("executor committed two commits\n", encoding="utf-8")
                    stderr_path.write_text("", encoding="utf-8")
                    combined_path.write_text(stdout_path.read_text(encoding="utf-8"), encoding="utf-8")
                    return RunnerResult(
                        exit_code=0,
            ...
            orch = Orchestrator(
                ...
                shell_runner=MultiCommitRunner(),
                review_service=fake,
            )
            orch.run_roadmap(roadmap)

            rows = {r["id"]: r for r in state.task_rows("multi-commit-merge")}
            self.assertEqual(rows["T1"]["state"], TaskState.MERGED.value)
            git(repo, "checkout", "--quiet", "integration/agentops-multi-commit")
            self.assertEqual((repo / "out.txt").read_text(encoding="utf-8"), "base\nfollow-up\n")
