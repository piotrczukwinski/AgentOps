                ...
                json.dumps(
                    {
                        ...
                        "model": task.model,
                        "reviewer": task.review.codex,
                        "reviewer_model": task.review.codex_model,
                        "reviewer_model_reasoning_effort": task.review.model_reasoning_effort,
        ...
        *,
        ...
        return "\n".join(
            [
                ...
                "- REQUEST_CHANGES: task is directionally OK but needs bounded repair.",
                "- BLOCK: unsafe, out-of-scope, reducing, or architecturally wrong.",
                "",
                "# Repair routing",
                "- For REQUEST_CHANGES, classify the repair scope in the JSON as `repair_scope`: `small`, `large`, or `architectural`.",
                "- `small`: local edits in the current diff/worktree; Codex should perform these directly in repair mode.",
                "- `large`: broad mechanical implementation; provide a precise executor prompt with files, invariants, and validation commands.",
                "- `architectural`: Codex must design the architecture and exact implementation plan; the executor may only apply that plan, not invent architecture.",
                "- If a change is small, do not write a vague executor prompt. Write exact required edits so Codex can apply them immediately.",
                "",
        ...
        *,
        ...
        return "\n".join(
            [
                ...
                "The reviewer requested changes. Codex is the default repairer for small/local fixes.",
                "Address every blocking issue below directly when the repair is local.",
                "If the repair is large or architectural, preserve Codex's architectural decisions in the implementation plan; do not delegate architecture to the executor.",
